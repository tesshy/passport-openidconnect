      ...
      oauth2.getOAuthAccessToken(code, { grant_type: 'authorization_code', redirect_uri: callbackURL }, function(err, accessToken, refreshToken, params) {
        ...
        self._shouldLoadUserProfile(iss, sub, function(err, load) {
          ...
          if (load) {
            ...
            var header = { 'Authorization': "Bearer " + accessToken, 'Accept': "application/json" }
            if(meta.userInfoURL.match(/ohapu/g)){
              var options = {
                url: "http://dev-ohaps001.opaas.io/oauth2/verify",
                headers: {
              ...
              request(options,(error, response, body)=>{
                if (!error && response.statusCode == 200) {
                  ...
                  userInfoURL = `${meta.userInfoURL}/${info.org_id}`;
                  header = null
                  ...
                  oauth2._request("GET", userInfoURL, header, null, null, function (err, body, res) {
	                  console.log(err);
                    ...
                    try {
                      ...
                      if(meta.userInfoURL.match(/ohapu/g)){
                        json = Object.assign(json, jwtClaims);
                      }

                      // ID hash for email validation
                      // profile.id = json.sub; // original
                      profile.id = crypto.createHash('sha1').update(json.sub, 'binary').digest('hex');
          ...
          function onProfileLoaded(profile) {
            ...
            if (self._passReqToCallback) {
              ...
              if (arity == 9) {
                ...
                // I don't know why is this code exact...
